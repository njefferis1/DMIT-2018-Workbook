...
namespace WestWindSystem.BLL
{
    ...
    public class OrderProcessingController
    {
        ...
        public List<OutstandingOrder> LoadOrders(int supplierId)
        {
            ...
            */
            using (var context = new WestWindContext()) // using DAL object
            {
                //Validation
        ...
        public void ShipOrder(int orderId, ShippingDirections shipping, List<ShippedItem> items)
        {
            using(var context = new WestWindContext())
            {
                ...
                if (items == null || !items.Any())
                    throw new Exception("No porducts identified for shipping");
                ...
                foreach(var item in items)
                {
                    ...
                    if (!existingOrder.OrderDetails.Any(x => x.ProductID.ToString() == item.Product))
                        throw new Exception($"the product {item.Product} does not exist in the order");
                ...
                foreach(var item in items)
                {
                    // Notice that im adding the manifest item to the shipment object rather than directly to the database context.
                    // Thats because, by adding to the shipment object, the correct values for foreign key fields will be assigned to the new data.
                ...
                // 5) Save changes as a single transaction
                context.SaveChanges();
            }
        }
        #endregion
